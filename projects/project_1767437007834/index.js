...
let reconnectTimer = null; // single scheduled reconnect timer
let moveInterval = null; // movement interval id (to clear between restarts)
let connectTimer = null; // connect timeout id (clear when needed)
let creating = false; // prevent re-entrant createBot calls
...
function createBot() {
  ...
  // use outer-scope connectTimer so we can clear it from other handlers
  ...
  function onSuccessfulConnect() {
    if (connectTimer) { clearTimeout(connectTimer); connectTimer = null; }
    // clear any pending reconnects once connected
  ...
  // movement interval — ensure only one interval exists at a time
  ...
  moveInterval = setInterval(() => {
    ...
    try {
      ...
      console.error('Error while moving bot:', e && e.message);
    }
  }, 3000);
}
...
function reconnect() {
  ...
  // ensure only one scheduled reconnect exists
  if (reconnectTimer) clearTimeout(reconnectTimer);
  reconnectTimer = setTimeout(() => {
    reconnectTimer = null;
    // cleanup previous bot and intervals before creating a new one
    try {
